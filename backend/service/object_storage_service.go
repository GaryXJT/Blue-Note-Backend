...
func (s *ObjectStorageService) GetFileURL(filePath string) string {
	...
	if strings.HasPrefix(filePath, "/uploads") || strings.Contains(filePath, "/uploads/") ||
		strings.HasPrefix(filePath, "uploads/") || strings.HasPrefix(filePath, "uploads") {

		// 确保路径格式正确，统一格式为 /uploads/...
		if !strings.HasPrefix(filePath, "/") {
			filePath = "/" + filePath
		}

		log.Printf("GetFileURL - 本地文件，返回: %s", filePath)
		return filePath
	}
	...
	url := fmt.Sprintf("https://%s/%s/%s",
		...
		s.bucketName,
		filePath)
	log.Printf("GetFileURL - 对象存储文件，返回: %s", url)
	return url
}
